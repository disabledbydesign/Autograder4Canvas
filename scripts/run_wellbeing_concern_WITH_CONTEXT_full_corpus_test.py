"""Test R (full-corpus extension, WITH CLASS CONTEXT): WELLBEING_CONCERN_PROMPT
on full 32 ES + 14 WB corpus, with the real class_reading prose injected into
the {class_context} slot.

Written 2026-05-11 as a parallel companion to
run_wellbeing_concern_full_corpus_test.py (the binary full-corpus script). That
script runs WELLBEING_CONCERN_PROMPT with class_context = "" (the original Test
R convention, matching the deployed binary tracks which did not inject class
context). This script runs the *same* prompt, *same* model, *same* corpus, but
fills the {class_context} slot with the real class reading loaded from
data/demo_baked/checkpoints/ethnic_studies_gemma12b_mlx_class_reading.json.

Why this matters. The output-format-bias paper's §IV.A.1 claims that adding
class context to the binary wellbeing classifier WORSENS it (12 flags, 0 true
positives, 6 confirmed false positives on protected students). That claim rests
on Test L data from March 2026 at a smaller scope. This new script confirms the
claim at full 46-student scope on the deployed prompt. Pair this run with the
class_context="" run (run_wellbeing_concern_full_corpus_test.py) to get the
same-corpus, same-scope, same-prompt comparison that isolates class_context as
the only varying input.

What this script does. Faithful copy of run_wellbeing_concern_full_corpus_test.py
with ONLY these changes:
  1. CLASS_CONTEXT_TEXT is loaded from
     data/demo_baked/checkpoints/ethnic_studies_gemma12b_mlx_class_reading.json
     (key: "class_reading") instead of being the empty string.
  2. Output filename: test_r_wellbeing_concern_WITH_CONTEXT_FULL_CORPUS_gemma12b_*.
  3. test_name field: "test_r_wellbeing_concern_with_context_full_corpus".
  4. description field: notes that class context is loaded (vs. empty in the
     paired run).
  5. class_context metadata field: short reference + char length, not the full
     prose (which is already in the per-row prompt and the source JSON file).

Everything else — prompt template, model, temperature, n_runs, 46-student
iteration, anti-bias regex post-processing, 0.7 threshold, output schema —
matches the binary script verbatim.

How to run.
    cd /Users/june/Documents/GitHub/Autograder4Canvas
    python scripts/run_wellbeing_concern_WITH_CONTEXT_full_corpus_test.py
    python scripts/run_wellbeing_concern_WITH_CONTEXT_full_corpus_test.py --smoke

Estimated runtime (full mode): ~10.5 hours. No time guards or cutoffs.

Output. JSON file at
data/raw_outputs/test_r_wellbeing_concern_WITH_CONTEXT_FULL_CORPUS_gemma12b_<date>_<time>.json
...
def load_class_reading() -> str:
    """Load the class_reading prose from the precomputed JSON checkpoint.

    Returns the prose string at the "class_reading" key. Raises if the file
    is missing or the expected key is absent — we want a hard failure here
    rather than silently falling back to an empty string, because that would
    collapse this run into the binary-no-context run and invalidate the
    comparison.
    """
    if not CLASS_READING_PATH.exists():
        raise FileNotFoundError(
            f"Class reading checkpoint not found at {CLASS_READING_PATH}. "
            "This script requires the precomputed class_reading prose to "
            "inject into the {class_context} slot. Without it, the run "
            "would be indistinguishable from the binary-no-context run."
        )
    data = json.loads(CLASS_READING_PATH.read_text())
    if "class_reading" not in data:
        raise KeyError(
            f"Expected 'class_reading' key in {CLASS_READING_PATH}; "
            f"found keys: {sorted(data.keys())}."
        )
    prose = data["class_reading"]
    if not isinstance(prose, str) or not prose.strip():
        raise ValueError(
            f"'class_reading' value in {CLASS_READING_PATH} is not a "
            f"non-empty string (got type={type(prose).__name__})."
        )
    return prose
...
# Intersectionality in Practice") and the same class_reading as the ES corpus
# students. UNLIKE the paired binary script, this run injects the real
# class_reading prose into {class_context} — that is the entire point of this
# run. signal_matrix_result is still the placeholder (Track B / binary
# wellbeing did not consume the non-LLM signal matrix).
ASSIGNMENT_PROMPT_TEXT = "Week 6 Discussion: Intersectionality in Practice"
CLASS_CONTEXT_TEXT = load_class_reading()
...
def save_results(
    ...
    filename = (
        f"test_r_wellbeing_concern_WITH_CONTEXT_FULL_CORPUS_{model_key}_"
        f"{date}_{time_tag}{smoke_tag}.json"
    )
    output = {
        "test_name": "test_r_wellbeing_concern_with_context_full_corpus",
        "description": (
            "WELLBEING_CONCERN_PROMPT WITH class context loaded from "
            "ethnic_studies_gemma12b_mlx_class_reading.json (rich DO-flag/"
            "DO-NOT-flag taxonomy from src/research/prompts.py) tested on the "
            "full 46-student corpus (32 ES synthetic ethnic-studies students "
            "S001-S032 + 14 WB wellbeing-signal cases WB01-WB14 from "
            "WELLBEING_SIGNAL_CASES in run_alt_hypothesis_tests.py). Companion "
            "to run_wellbeing_concern_full_corpus_test.py, which runs the same "
            "prompt with class_context=\"\" (empty). The only varying input "
            "between the two runs is the {class_context} slot, isolating its "
            "effect on the binary wellbeing classifier. Confirms the §IV.A.1 "
            "claim (Test L, March 2026) at full 46-student scope on the "
            "deployed prompt: adding class context worsens the binary "
            "classifier."
        ),
        ...
        "class_context": (
            f"loaded from {CLASS_READING_PATH.name} "
            f"({len(CLASS_CONTEXT_TEXT)} chars); source key 'class_reading'"
        ),
        "class_context_source_path": str(CLASS_READING_PATH),
        "class_context_chars": len(CLASS_CONTEXT_TEXT),
...
def run_test(
    ...
    print(f"\n{'=' * 60}")
    print(f"  TEST R (FULL CORPUS, WITH CLASS CONTEXT): WELLBEING_CONCERN_PROMPT")
    print(f"  Scope: 32 ES (S001-S032) + 14 WB (WB01-WB14) = 46 students")
    print(f"  class_context: {len(CLASS_CONTEXT_TEXT)} chars from "
          f"{CLASS_READING_PATH.name}")
    if smoke:
    ...
    for run_idx in range(1, n_runs + 1):
        ...
        for sid, name, text, pattern_or_signal, expected, source in all_cases:
            ...
            results.append(
                {
                    "test_variant": "R_with_context_full_corpus",
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Test R full-corpus extension WITH class context: "
            "WELLBEING_CONCERN_PROMPT on the 32 ES + 14 WB corpus (46 "
            "students), with the real class_reading prose injected into "
            "the {class_context} slot."
        )
    )
    parser.add_argument(
